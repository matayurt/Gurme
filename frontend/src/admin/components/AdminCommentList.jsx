...
import { getUnapprovedReplies, approveReply, deleteReply } from '../../actions/replyActions';
import { getRestaurants } from '../../actions/restaurantActions';
import { getCategories } from '../../actions/categoryActions';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCheck, faTrash, faStar, faEdit } from '@fortawesome/free-solid-svg-icons';
...
const AdminCommentList = ({ darkMode }) => {
  ...
  const { loading: loadingReplies, error: errorReplies, replies = [] } = replyList;
  ...
  const approveHandler = (commentId) => {
    dispatch(approveComment(commentId))
      ...
      .catch(() => {
        toast.error('Yorum onaylanamadı. Lütfen tekrar deneyin.', {
          position: 'top-right',
          autoClose: 1000,
          hideProgressBar: false,
          closeOnClick: true,
  ...
  const approveReplyHandler = (replyId) => {
    dispatch(approveReply(replyId))
      .then(() => {
        toast.success('Yanıt başarıyla onaylandı!', {
          position: 'top-right',
          autoClose: 1000,
          hideProgressBar: false,
          closeOnClick: true,
      ...
      .catch(() => {
        toast.error('Yanıt onaylanamadı. Lütfen tekrar deneyin.', {
          position: 'top-right',
          autoClose: 1000,
          hideProgressBar: false,
          closeOnClick: true,
  ...
  const approveSelectedHandler = () => {
    selectedComments.forEach((commentId) => approveHandler(commentId));
    setSelectedComments([]);
  };
  ...
  return (
    <div className={`container mx-auto p-4 ${darkMode ? 'text-white' : 'text-black'}`}>
      <div className={`p-6 rounded-lg shadow-lg mb-6 ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
        <h2 className={`text-3xl font-semibold text-center ${darkMode ? 'text-white' : 'text-gray-800'}`}>Onay Bekleyen Yorumlar</h2>
      </div>
...
<div className="mt-4 flex space-x-2">
                      <button
                        onClick={(e) => { e.stopPropagation(); approveHandler(comment._id); }}
                        className={`px-4 py-2 rounded-lg shadow transition duration-300 ${darkMode ? 'bg-green-600 hover:bg-green-500 text-white' : 'bg-green-500 hover:bg-green-600 text-white'}`}
                      >
